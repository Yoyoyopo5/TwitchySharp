...
public record ModifyChannelInformationRequestData
{
    ...
    /// The game is not updated if the ID isn�ft a game ID that Twitch recognizes. 
    /// To unset this field, use <c>"0"</c> or <see cref="string.Empty"/>.
    /// </summary>
    public GameId? GameId { get; init; }
    /// <summary>
    /// The user�fs preferred language. 
    /// Set the value to an ISO 639-1 two-letter language code (for example, en for English). 
    /// Set to "other" if the user�fs preferred language is not a Twitch supported language. 
    /// The language isn�ft updated if the language code isn�ft a Twitch supported language.
    /// </summary>
    public LanguageCode? BroadcasterLanguage { get; init; }
    /// <summary>
    /// The title of the user�fs stream. 
    ...
    /// <summary>
    /// List of labels that should be set as the Channel�fs CCLs.
    /// </summary>
