...
public sealed record ShieldModeEnd(UserId BroadcasterUserId, UserId ModeratorUserId)
    : EventSubSubscriptionTypeSpecification, IConditionConstructable<ShieldModeEnd>
{
    public override EventSubSubscriptionType Type => EventSubSubscriptionType.ShieldModeEnd;
    public static EventSubSubscriptionType SubscriptionType => EventSubSubscriptionType.ShieldModeEnd;
    ...
    public static Validation<ShieldModeEnd> FromCondition(IReadOnlyDictionary<ConditionKey, string> condition)
        => condition
            .GetRequiredValue(new("broadcaster_user_id"), out UserId BroadcasterUserId, value => new(value))
            .GetRequiredValue(new("moderator_user_id"), out UserId ModeratorUserId, value => new(value))
            .Map(_ => new ShieldModeEnd(BroadcasterUserId, ModeratorUserId));
}
